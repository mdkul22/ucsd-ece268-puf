...
`timescale 1ns/1ps
module puf_axi_interface_AXILiteS_s_axi
#(parameter
...
localparam
    ...
    ADDR_TUNERAXI_V_DATA_1      = 7'h2c,
    ADDR_TUNERAXI_V_CTRL        = 7'h30,
    ADDR_OUTPUT1AXI_V_DATA_0    = 7'h34,
    ADDR_OUTPUT1AXI_V_CTRL      = 7'h38,
    ADDR_OUTPUT2AXI_V_DATA_0    = 7'h3c,
    ADDR_OUTPUT2AXI_V_CTRL      = 7'h40,
    ADDR_STARTAXI_V_DATA_0      = 7'h44,
    ADDR_STARTAXI_V_CTRL        = 7'h48,
    WRIDLE                      = 2'd0,
    WRDATA                      = 2'd1,
    WRRESP                      = 2'd2,
    WRRESET                     = 2'd3,
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_TUNERAXI_V_DATA_0: begin
                    rdata <= int_tuneraxi_V[31:0];
                end
                ADDR_TUNERAXI_V_DATA_1: begin
                    rdata <= int_tuneraxi_V[39:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_tuneraxi_V[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TUNERAXI_V_DATA_0)
            int_tuneraxi_V[31:0] <= (WDATA[31:0] & wmask) | (int_tuneraxi_V[31:0] & ~wmask);
    end
end

// int_tuneraxi_V[39:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_tuneraxi_V[39:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TUNERAXI_V_DATA_1)
            int_tuneraxi_V[39:32] <= (WDATA[31:0] & wmask) | (int_tuneraxi_V[39:32] & ~wmask);
    end
end
